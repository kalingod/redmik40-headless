...
static int upload_texture(struct vk_ctx *ctx)
{
    ...
    mapped = NULL;
    printf("texture_pattern=vertical_stripes texture=%ux%u stripe_width=%u bytes=%llu\n",
           TEXTURE_WIDTH, TEXTURE_HEIGHT, STRIPE_WIDTH,
                           ...
                           unsigned int frame)
{
    ...
    printf("kgsl_textured_render frame=%u\n", frame);
    return 0;
}

static void expected_bgra_for_x(uint32_t x, uint32_t width, uint8_t out[4])
{
    uint32_t tex_x = (uint32_t)(((uint64_t)x * TEXTURE_WIDTH) / width);
    bool red = ((tex_x / STRIPE_WIDTH) & 1u) == 0;
...
static int verify_texture_samples(const struct drm_buffer *buf, unsigned int frame)
{
    const uint32_t stripes = TEXTURE_WIDTH / STRIPE_WIDTH;
    ...
    for (uint32_t i = 0; i < stripes; i++) {
        uint32_t x = (uint32_t)(((uint64_t)(2 * i + 1) * buf->width) / (2 * stripes));
        const uint8_t *px = (const uint8_t *)buf->map + (size_t)y * buf->pitch + (size_t)x * 4;
        uint8_t expected[4];
        bool ok;

        expected_bgra_for_x(x, buf->width, expected);
        ok = memcmp(px, expected, 4) == 0;
        printf("sample frame=%u stripe=%u x=%u y=%u bgra=%02x %02x %02x %02x expected=%02x %02x %02x %02x ok=%s\n",
               frame, i, x, y,
               px[0], px[1], px[2], px[3],
    ...
    printf("texture_sample_pass=%u/%u frame=%u\n", pass, stripes, frame);
    return pass == stripes ? 0 : -1;
}
...
{
    const char *vert_path = "/data/experiments/alioth_vulkan_textured_quad.vert.spv";
    const char *frag_path = "/data/experiments/alioth_vulkan_textured_quad.frag.spv";
    ...
    if (render_textured(&vk, &images[0], 0) != 0)
        goto cleanup;
    if (verify_texture_samples(&buffers[0], 0) != 0)
        goto cleanup;
    ...
    for (unsigned int i = 0; i < flips; i++) {
        ...
        if (render_textured(&vk, &images[next], i + 1) != 0)
            goto cleanup;
        if (verify_texture_samples(&buffers[next], i + 1) != 0)
            goto cleanup;
    ...
    printf("drm_prime_kgsl_textured_pageflip=PASS submitted=%u events=%u\n",
           flips, flip.seen);
    rc = 0;
